...
function printSubmit() {
	if($_SESSION['SFM']['InputErr']) {
		// エラーがある場合のHTML出力
		$submit = <<< EOD
<p class="txtLL p20">入力された内容に不足があります。戻るボタンを押して必須項目を入力してください。</p>
<p id="confirm-button" class="submit-container"><span><button type="button" onclick="history.back()" style=" border:0; background:none;" /><img src="img/formBackBtn.gif" width="68" height="26" alt="戻る" /></button></span></p>
EOD;
	} else {
		// 項目が正しい場合のHTML出力
		$submit = <<< EOD
<p class="txtLL pt20 pb20">入力が正しければ、送信ボタンを押して下さい。</p>
<input type="hidden" name="mode" id="mode" value="SEND" />
<p id="confirm-button" class="submit-container"><span><input type="button" onclick="history.back()" value="戻る" />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<input type="submit" value="この内容でメール送信" /></span></p>
EOD;
	}
	return $submit;
}
